    ...
    if (
      ...
      chrome.scripting.executeScript({
        ...
        function: () => {
          ...
          const branchName = `${type}/${issue}-${summary}`
            .slice(0, 50)
            .replace(/-$/, "");

          window.open(
            "https://github.com/ds-itu-frontend-service/danskespil-website/branches?newBranch=" + `${type}/${issue}-${summary}`.slice(0, 50).replace(/-$/, "")
          );
        },
      })
    }
